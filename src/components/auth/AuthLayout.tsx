...
const AuthLayout: React.FC<AuthLayoutProps> = ({ 
  ...
  const stats = [
    { icon: BookOpen, label: 'Academic Programs', value: '25+', color: 'text-blue-400' },
    { icon: Users, label: 'Faculty Members', value: '200+', color: 'text-green-400' },
    { icon: Award, label: 'Research Projects', value: '150+', color: 'text-yellow-400' },
    { icon: Globe, label: 'Industry Partners', value: '100+', color: 'text-purple-400' }
  ];
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute top-1/2 left-1/4 w-24 h-24 bg-accent/30 rounded-full blur-2xl animate-float" style={{ animationDelay: '4s' }} />
      </div>
          ...
          <div className="auth-content space-y-8 text-white">
            ...
            {showBackButton && (
              <Link to="/">
                <Button 
                  variant="ghost" 
                  size="sm"
                  className="text-white hover:bg-white/20 hover:text-white border border-white/30 backdrop-blur-sm transition-all duration-300 hover:scale-105"
                >
            ...
            <div className="flex items-center space-x-4">
              <img 
                src={logoImage} 
                alt="IIT Patna Logo" 
                className="h-16 w-16 lg:h-20 lg:w-20 object-contain animate-pulse-glow"
              />
              <div>
            ...
            <div className="grid grid-cols-2 gap-4 lg:hidden">
              {stats.map((stat, index) => {
                ...
                return (
                  <div 
                    key={stat.label}
                    className="auth-stats bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20 hover:bg-white/20 transition-all duration-300 hover:scale-105"
                  >
            ...
            <div className="hidden lg:grid grid-cols-2 gap-6">
              {stats.map((stat, index) => {
                ...
                return (
                  <div 
                    key={stat.label}
                    className="auth-stats bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/30 hover:bg-white/20 transition-all duration-500 hover:scale-110 group cursor-pointer"
                  >
                    <div className="flex items-center space-x-4">
